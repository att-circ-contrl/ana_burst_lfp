function [timelock] = ft_spiketriggeredaverage4(cfg, data)
% BV: spkes at boundaries can now be truncated segments (with nans)
% 
...
%
% BV: updated so that we offset the timwin for spikes around borders
...
cfg.boundaryaction      = ft_getopt(cfg,'boundaryaction','maxperiod');

% ensure that the options are valid
%BV: add new config to add in multiple toi
...
cfg = ft_checkconfig(cfg, 'allowed', {'boundaryaction','eventtime','toi','timwin', 'spikechannel', 'channel', 'keeptrials', 'feedback', 'latency', 'trials', 'warning', 'progress'});
...
%spiketrialindex = cell(1,ntrial);

cumsum = zeros(nchansel, numsmp);
cumcnt = zeros(nchansel, numsmp);

% BV: select which function to use for the boundary
if strcmp(cfg.boundaryaction,'shift')
    error('not finished')
    %boundaryfun = @shiftsegment;
    %ignoresegment = false;
elseif strcmp(cfg.boundaryaction,'excise')
    %boundaryfun = @excisesegment;
    ignoresegment = false;
elseif strcmp(cfg.boundaryaction,'ignore')
    %boundaryfun = []; 
    ignoresegment = true;
else
    error('unrecognized boundary function')
end
...
for i=1:ntrial
  ...
  %figure out the latency indices for this trial
  nsmp = size(data.trial{i},2);
  beglatency = max(1, nearest(data.time{i},cfg.latency(1)) );
  endlatency = min(nsmp, nearest(data.time{i},cfg.latency(2)) );
    
  %only consider spike if it falls within latenyc
  %withinlatency = spikesmp >= beglatency & spikesmp <= endlatency;
  withinlatency = data.time{i}(spikesmp) >= cfg.latency(1) & data.time{i}(spikesmp) <= cfg.latency(2);
  spikesmp(~withinlatency) = [];
  spikecnt(~withinlatency) = [];
  ...
  %notwithinlatency = false(size(spikesmp));
  ...
  for j=1:length(spikesmp)
    ...
    %BV: cut out anything not within the latency window
    
    if ignoresegment
         if begsmp < beglatency
          % a possible alternative would be to pad the begin with nan
          % this excludes the complete segment
          continue
        elseif endsmp > endlatency
     ...
     elseif strcmp(cfg.boundaryaction,'excise')
        ...
        endsegpad = [];
        if begsmp < beglatency 
          offset = beglatency - begsmp;
          begsmp = begsmp + offset;
          begsegpad = nan(numel(chansel),abs(offset));
        end

        if endsmp > endlatency 
          offset = endlatency - endsmp;
          endsmp = endsmp + offset;
          endsegpad = nan(numel(chansel),abs(offset));
        end

        segment = [begsegpad, data.trial{i}(chansel,begsmp:endsmp), endsegpad];  
    end
    %}

    if strcmp(cfg.keeptrials, 'yes')
      %spiketrialindex{i}(j,1) = ceil(numel(segment)/2) - offset;
...
if (strcmp(cfg.keeptrials, 'yes'))
  ...
  %timelock.trialindex     = cat(1, spiketrialindex{:});
...
%BV: add this function here because fieldtrip doesnt want to access it for
%some reason...
