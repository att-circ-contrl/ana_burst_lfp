function ana_sta_pow_timeresolved_noShuffle(list,datadir,figpath)
...
if 1
     ...
     for icell=1:3
        ...
        for ifreq=1:2
            ...
            selfreq = selInBounds(freqoi2,foi(ifreq,:));
...
%                 dat2shf = tmpshf(:,ii+1:end,:); 
            ...
            fcfg.ylabel = 'norm rec';
            fcfg.plotInSubplots = 1;
            fcfg.subplotrc = [nr nc 1];
            fcfg.plot1 = (ifreq-1)*2*nc + icell;
            fcfg.plot2 = (ifreq-1)*2*nc + icell + nc;

            %[hax(icell,ifreq), haxd(icell,ifreq),datout] = plot_timeresolved_difference(dat1,dat2,fcfg);
            %[hax(icell,ifreq), haxd(icell,ifreq),datout] = plot_timeresolved_difference2(dat1,dat2,fcfg);
            [hax(icell,ifreq), haxd(icell,ifreq),datout] = plot_difference_multcompcorr(dat1,dat2,fcfg);
            out_all{icell,ifreq} = datout;
    ...
    %save
    sname = [figpath '/pow_slidewin_fft_ncycle' num2str(ncycle) '_' normstr '_multcomp' num2str(doMultComp)];
    save2pdf(sname,gcf)
    
    sname2 = [figpath '/pow_slidewin_datout_ncycle' num2str(ncycle) '_' normstr '.mat'];
    save(sname2,'out_all','fcfg')
